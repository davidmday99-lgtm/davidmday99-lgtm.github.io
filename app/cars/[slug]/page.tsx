...
export function generateMetadata({
  ...
  const listing =
    demoListings.find((item) => item.slug === params.slug) ?? demoListings[0];
  ...
  return {
    title: `${listing.year} ${listing.name} | OwnerOnly Cars`,
    description,
    openGraph: {
      title: `${listing.year} ${listing.name}`,
      description,
...
export default function VehiclePage({ params }: { params: { slug: string } }) {
  ...
  const specs = [
    ...
    ['Body style', listing.bodyStyle, Car],
    ['Year', String(listing.year), Calendar],
    ['Location', listing.location, MapPin],
  ] as const;
  return (
    <>
      <SiteHeader />
      <main className="bg-[#f8f4e9]">
        <div className="mx-auto max-w-7xl px-5 py-8 sm:px-8">
          <p className="text-sm text-slate-600">
            <a className="font-bold hover:underline" href="/search">
              Browse cars
            </a>{' '}
          ...
          <div className="mt-6 grid gap-6 lg:grid-cols-[1.5fr_.8fr]">
            <section>
              <div className="relative overflow-hidden border-2 border-navy bg-white shadow-[8px_8px_0_rgba(7,28,44,.18)]">
                <img
                  alt={`${listing.year} ${listing.name} demonstration listing`}
                  className="aspect-[16/10] size-full object-cover"
                  src={listing.image}
                />
                <Badge className="absolute left-4 top-4 rounded-none bg-navy text-white">
                  Demo listing
                </Badge>
              </div>
              <div className="mt-6 grid grid-cols-3 gap-3">
                {[listing.image, listing.image, listing.image].map(
                  (image, index) => (
                    <div
                      className="overflow-hidden border-2 border-navy bg-white"
                      key={index}
                    >
                      <img
                        alt={`Vehicle gallery view ${index + 1}`}
            ...
            <aside className="h-fit border-2 border-navy bg-white p-6 shadow-[8px_8px_0_#16b9ad] lg:sticky lg:top-24">
              ...
              <h1 className="mt-1 text-3xl font-black uppercase leading-none tracking-[-0.045em] text-navy">
                {listing.name}
              </h1>
          ...
          <section className="mt-14 grid gap-8 lg:grid-cols-[1fr_.65fr]">
            <div>
              <h2 className="text-3xl font-black uppercase tracking-[-0.045em] text-navy">
                Vehicle details
              </h2>
              <div className="mt-5 grid border-2 border-navy bg-white sm:grid-cols-2">
                {specs.map(([label, value, Icon], index) => (
                  <div
                    className={`flex gap-4 p-5 ${index % 2 ? 'sm:border-l' : ''} ${index > 1 ? 'border-t' : ''} border-slate-300`}
                    key={label}
                  >
              ...
              <h2 className="mt-10 text-3xl font-black uppercase tracking-[-0.045em] text-navy">
                Seller’s description
              </h2>
              <p className="mt-4 max-w-3xl leading-8 text-slate-700">
                Well-kept, everyday vehicle with regular maintenance and a clean
              ...
              >
                <div className="flex items-start gap-4">
                  <div className="bg-teal-100 p-3 text-teal-800">
                    <FileSearch className="size-6" />
                  </div>
                  <div>
                    <p className="text-xs font-black uppercase tracking-[0.18em] text-teal-800">
                      Seller-provided · third party
                    </p>
                    <h2
                      className="mt-1 text-2xl font-black uppercase text-navy"
                      id="vehicle-history-heading"
                    >
                      CARFAX vehicle history
                    </h2>
                  </div>
                </div>
                <div className="mt-5 border-l-4 border-slate-300 bg-slate-50 p-4">
                  <p className="font-bold text-navy">
                    No CARFAX report is attached to this demonstration listing.
                  </p>
                  <p className="mt-2 text-sm leading-6 text-slate-600">
                    When a seller adds an official CARFAX link, always confirm
                    the report VIN matches the vehicle. A report may not contain
                    every accident, repair, or title event.
                  </p>
                </div>
                <a
                  className="mt-5 inline-flex items-center gap-2 border-2 border-navy bg-navy px-4 py-3 text-sm font-black uppercase text-white hover:bg-teal-800"
                  href={CARFAX_REPORTS_URL}
                  rel="noreferrer"
                  target="_blank"
                >
                  Get a report from CARFAX <ExternalLink className="size-4" />
                </a>
                <p className="mt-3 text-xs leading-5 text-slate-500">
                  Direct non-affiliate link. OwnerOnly receives no payment.
                  CARFAX is not an OwnerOnly verification badge, and OwnerOnly
                  is not affiliated with or endorsed by CARFAX.
                </p>
              </section>
            </div>
            <div className="border-2 border-navy bg-[#f6b82b] p-6 shadow-[6px_6px_0_rgba(7,28,44,.18)]">
              ...
              <ul className="mt-5 space-y-3 text-sm leading-6 text-navy/85">
                ...
                <li>• Never send gift cards or wire money to hold a car.</li>
                <li>• Verify the VIN on the vehicle matches the title.</li>
              </ul>
              <a
