...
export function AuctionSourceCard({ source }: { source: AuctionSource }) {
  ...
  const isIndependent = source.sourceType === 'independent-broker';
  return (
    <article className="flex h-full flex-col border-2 border-navy bg-white p-6 shadow-[6px_6px_0_rgba(7,28,44,.14)]">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {isIndependent && <Badge className="rounded-none border-amber-700 bg-amber-100 text-amber-900">Independent broker</Badge>}
      </div>
      ...
      <div className="mt-auto flex flex-wrap gap-3 pt-6">
        <a className="inline-flex items-center gap-2 bg-navy px-4 py-3 text-xs font-black uppercase tracking-wide text-white hover:bg-teal-800" href={source.officialUrl} rel="noopener noreferrer" target="_blank"><Globe2 className="size-4" /> {isIndependent ? 'Open platform' : 'Official source'} <ExternalLink className="size-3" /></a>
