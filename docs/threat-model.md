...
| Public-auction payment phishing                | No bid/payment form in the directory, external-link warning, never store third-party auction credentials, official operator links only                                       |
| Bid race, replay, or closing-time manipulation | Serializable row lock, server clock, idempotency key, monotonic sequence, atomic extension rule, immutable audit event                                                       |
| Reserve or maximum-bid leakage                 | Server-only columns, redacted projections/realtime events/logs, authorization tests, no client-side reserve comparison                                                       |
| Shill bidding or seller self-bid               | Seller/connected-account prohibition, ownership graph and device/contact signals, human review, suspension and appeal                                                        |
| Auction cancellation or moderator abuse        | Restricted actions, documented reason, dual control for exceptional close/reopen, bidder notice, append-only audit history                                                   |
