...
| private_auctions           | Approved owner listing; start/end/extended end; starting bid; private reserve; increment; draft/review/scheduled/live/ended/cancelled state; winner/result                                             |
| private_auction_bids       | Auction; verified bidder; private maximum; displayed amount; server sequence and timestamp; idempotency key; accepted/rejected state                                                                   |
| private_auction_extensions | Auction; triggering bid; prior/new end; rule version; append-only timestamp                                                                                                                            |
...
- A private auction references exactly one approved owner listing and VIN. Only one scheduled/live private auction may exist for that vehicle.
- Reserve and bidder-maximum columns are excluded from every public view and realtime publication. Bid idempotency keys are unique per bidder and request.
- Live bid writes lock the auction row and use a serializable transaction. Server sequence and displayed amount are monotonic; accepted bids cannot be edited or deleted.
...
- Verified bidder: may submit a bid through one server mutation only; cannot directly select private maxima, reserves, other bidders, or unpublished auction records.
